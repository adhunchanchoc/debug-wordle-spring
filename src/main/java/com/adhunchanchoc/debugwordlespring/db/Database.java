...
public class Database {
    ...
    private static final String INSERT_SQL = """
            INSERT INTO activity (userId, guess, word, time_stamp, attempt)
            VALUES (?, ?, ?, ?, ?)
            """;
    private static final String SELECT_ALL_SQL = """
            SELECT * FROM activity WHERE userId = ? ORDER BY attempt
            """;
    ...
    public void insert(DBEntry entry) {
        ...
        if(entry.user()!=null && attempt == 0){
            attempt = jdbcTemplate.queryForObject(COUNT_SQL,Integer.class,entry.user(),entry.word());
        }
        jdbcTemplate.update(INSERT_SQL, entry.user(), entry.guess(), entry.word(), Instant.now(), attempt); //attempts calculated above
    }
